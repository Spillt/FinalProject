...
<div class="container">
  ...
  <c:forEach var="a" items="${requestScope.myQnA}">
  	<c:if test="${a.question eq q.question_no }">
  		...
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/1" checked>${q.choice1 }</div>  
  		...
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/2" checked>${q.choice2 }</div>
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/3">${q.choice3 }</div>
  		</c:if>
  		<c:if test="${a.answer1 eq 3 }">
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/1">${q.choice1 }</div>  
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/2">${q.choice2 }</div>
  		<div class="anser"><input type="radio" name="${q.question_no }" class = "checkSelect" value = "${q.question_no }/3" checked>${q.choice3 }</div>
  		</c:if>
  	</c:if>
  </c:forEach>
  <br>
...
<script type="text/javascript">
	...
	function radioboxCheck(){
		 if($("input:radio:checked").length != $("input:radio").length/3){
			...
			document.location.href="updateMatchqna.do?check="+send_array+"&userId=${user_id}"; 
		}
