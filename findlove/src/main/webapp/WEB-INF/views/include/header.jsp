...
<!-- 파비콘 -->
<link rel="shortcut icon" type="image/x-icon" href="/findlove/resources/img/favicon.ico">
		...
		<div class="container">
			...
			<div id="container2">
				<span id="menu-btn" class="navbar-toggler navbar-toggler-right"
					...
					class="material-icons">menu</i></span> <span
			...
			<div class="collapse navbar-collapse" id="navbarResponsive">
				<ul class="navbar-nav text-uppercase ml-auto">
					...
					<li class="nav-item"><a class="nav-link"
						href="#" id = "matching">매칭 게시판</a></li>
					<li class="nav-item"><a class="nav-link"
						href="#" id ="openProfileList">프로필 게시판</a></li>
					<li class="nav-item"><a class="nav-link"
						href="#" id = "openFreeboardList">자유 게시판</a></li>
					<li class="nav-item"><a class="nav-link"
						href="#" id = "attendancelist">이벤트 게시판</a></li>
					<li class="nav-item"><a class="nav-link"
						href="#" id = "openFaqList">FaQ</a></li>
				</ul>
			</div>
		</div>
			...
			<c:choose>
				<c:when test="${sessionScope.user_id ne null }">
					...
					<c:if test="${sessionScope.user_id ne 'admin' }">
						<div id="headerKonpeitoCnt" class="nav-text">별사탕 : ${sessionScope.konpeito_cnt }</div>
						<div id="headerPointCnt" class="nav-text">포인트 : ${sessionScope.point_cnt }</div>
					</c:if>
				...
				<c:otherwise>
					...
					<form class="form-container" name="form1" method="post" id=loginform>
						<input type="text" id="user_id" name="user_id"
							...
							class="form-control" placeholder="패스워드" onkeypress="JavaScript:press(event)">
				...
				<c:when test="${sessionScope.user_id ne null }">
					<c:if test="${sessionScope.user_id ne 'admin' }">
						<div class="i-a-group">
							...
							<a href="myinfo.do?user_id=${sessionScope.user_id }">마이 페이지</a>
						</div>
						<div class="i-a-group">
							<i class="material-icons">face</i><a
								href="openSendMessageList.do?user_id=${sessionScope.user_id }">나의 매칭</a>
						</div>
					</c:if>
					<c:if test="${sessionScope.user_id eq 'admin' }">
						<div class="i-a-group">
							<i class="material-icons">supervisor_account</i>
							<a href="userlist.do?pageNo=1">회원관리</a>
						</div>
						<div class="i-a-group">
					 ...
					 $.ajax({
							...
							success : function(response){
								 if(response != '0'){
									 swal({
											title:"warning!",
											text:"아이디 혹은 비밀번호가 틀립니다 다시 입력해주세요.",
											icon:"error",
											});	 					
										$("#user_id").focus(); //입력포커스
										return false;
	...
	<script>		
		...
		$("#matching").on('click',function() {
			if(userId == ""){
				...
				document.location.href = "matchstartview.do?userId=" + userId;
			}				
		});
		...
		$("#attendancelist").on('click',function() {
			if(userId == ""){
				...
				document.location.href = "attendancelist.do?userId=${sessionScope.user_id}";
			}				
		});
		
		$("#openFreeboardList").on('click',function() {
			if(userId == ""){
				alert("로그인이 필요한 게시판입니다.");
			}else{
				document.location.href = "openFreeboardList.do";
			}				
		});
		
		$("#openProfileList").on('click',function() {
			if(userId == ""){
				alert("로그인이 필요한 게시판입니다.");
			}else{
				document.location.href = "openProfileList.do";
			}				
		});
		...
		$("#openFaqList").on('click',function() {
			if(userId == ""){
				alert("로그인이 필요한 게시판입니다.");
			}else{
				document.location.href = "openFaqList.do?faqGB=계정";
			}				
		});
		</script>
		<script>
		if('${sessionScope.user_id}' != "" && '${sessionScope.user_id}' != "admin") {
			
			$.ajax({
				url : "realtimepointcnt.do",
				data : {
					userId : '${sessionScope.user_id }'
				},	
				success : function(response){
					$('#headerKonpeitoCnt').text("별사탕: " + response.konpeito);
					$('#headerPointCnt').text("포인트: " +response.point);
				}
			});
		}
