	...
	<div class="container">
		...
		<ul class="pagination justify-content-center">
           <li class="page-item">
                ...
                <c:if test="${map.profilePager.curBlock > 1}">
                    <a href="javascript:list('1')" class="page-link">처음</a>
                </c:if>
            </li>
                ...
                <c:if test="${map.profilePager.curBlock > 1}">
                    <a href="javascript:list('${map.profilePager.prevPage}')"class="page-link">이전</a>
                </c:if>
                 </li>
                
                
                <!-- **하나의 블럭에서 반복문 수행 시작페이지부터 끝페이지까지 -->
                <c:forEach var="num" begin="${map.profilePager.blockBegin}" end="${map.profilePager.blockEnd}">
                    <!-- **현재페이지이면 하이퍼링크 제거 -->
                    <c:choose>
                        ...
                        <c:otherwise>
                            <li class="page-item"><a href="javascript:list('${num}')" class="page-link">${num}</a></li>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                ...
                <c:if test="${map.profilePager.curBlock <= map.profilePager.totBlock}">
                    <a href="javascript:list('${map.profilePager.nextPage}')" class="page-link">다음</a>
                </c:if>
                </li>
                
                <li class="page-item">
                <!-- **끝페이지로 이동 : 현재 페이지가 전체 페이지보다 작거나 같으면 [끝]하이퍼링크를 화면에 출력 -->
                <c:if test="${map.profilePager.curPage <= map.profilePager.totPage}">
                    <a href="javascript:list('${map.profilePager.totPage}')" class="page-link">끝</a>
                </c:if>
                </li>
